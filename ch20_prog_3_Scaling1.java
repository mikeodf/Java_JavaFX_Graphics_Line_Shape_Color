...
Scale the object by multipliers scale_x in the horizontal direction and  scale_y in the vertical direction  .

scale( scale_x,  scale_y, pivotX, pivotY):
Scale the object about the given pivot point.
...
public class Scaling1 extends Application { 
   ...
   public void start(Stage stage) {  
     ...
     //******************************************************************* 
      // Construction/instantiation to enable scaling to take place    
      Scale scale = new Scale();             

      //Scale: Setting the horizontal and vertical amplification. 
      scale.setX(2); 
      scale.setY(0.5);     
      scale.setPivotX(0);
      scale.setPivotY(0);  
     
      //Adding scaling to circle2, rect3,  rect4 
      circle2.getTransforms().addAll(scale); 
      rect3.getTransforms().addAll(scale); 
      rect4.getTransforms().addAll(scale);

      Group root = new Group(circle1,circle2, rect1, rect2, rect3, rect4); 
      Scene scene = new Scene(root, 660, 380);  
      stage.setTitle("Scaling transformation (X*2, Y*0.5"); 
